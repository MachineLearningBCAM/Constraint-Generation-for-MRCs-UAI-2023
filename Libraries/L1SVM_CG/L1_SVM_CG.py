...
#---Infinite loop until all the variables have non reduced cost
    ...
    #---Solve the problem to get the dual solution
        ...
        #---Compute all reduce cost and look for variable with negative reduced costs
            ...
            RC_array         = alpha*ones_P[len(columns_to_check)-1] - np.abs( np.dot(X_train[:, np.array(columns_to_check)].T, RC_aux) ) if not is_sparse else alpha*ones_P[len(columns_to_check)-1] - np.abs( X_train[:, np.array(columns_to_check)].T.dot( RC_aux) )
